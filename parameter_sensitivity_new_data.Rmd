...
Data from a double-tagging study (GLS light logger & GPS on the same animal) are analysed using a new method of light-based geolocation and the results presented in Bindoff et al. (2017). Tracks are reproduced using 3 shading x 3 movement parameters, and the results presented as deviation from the GPS track as per Results in Bindoff et al.
...
The plot above shows the time series of light observations, day by day. The pixels represent the observed light, so white pixels are bright daylight and black pixels are complete darkness. The seal spends so much time diving that the twilight times cannot be determined precisely, and the light sensor on this tag picked up moonlight quite easily, so phases of the moon will have an effect on our threshold values.
