...
path <- na.omit(gdat[,c("Day", "Longitude", "Latitude")])
path <- group_by(path, Day) %>% summarise(Lon = mean(Longitude), Lat = mean(Latitude))
...
#  raw GPS positions
gps <- "09-78524.GPS.raw.track.txt"
gdat <- read_tsv(gps) %>% select(Name, Day, Time, Longitude, Latitude, Longitude)
...
```{r calibrate95}
zen <- 97
day <- as.POSIXct("2009-10-20 06:00:00", "UTC")
d.lig$Date <- d.lig$Date - 60*60
thresh <- calibrate(d.lig, day, 70.9, -48.0, offset = 3, zen) * 1.10
```
